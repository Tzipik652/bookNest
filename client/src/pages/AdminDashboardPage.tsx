...
export function AdminDashboardPage() {
  ...
  const handleUpdateUser = async () => {
    ...
    try {
      ...
      // עדכון מיידי של ה-UI המקומי (אופציונלי, אבל מומלץ לחוויה חלקה)
  ...
  // ✅ תיקון 1: עדכון ה-State מיד לאחר המחיקה
  const handleDeleteBook = async (bookId: string) => {
    if (window.confirm('Are you sure you want to delete this book? This will also remove all associated comments and favorites.')) {
      try {
        await deleteBook(bookId); // המתן למחיקה בשרת
        
        // עדכון מיידי של הטבלה בצד לקוח (לפני רענון מהשרת)
        setBooks(prevBooks => prevBooks.filter(book => book._id !== bookId));
        
        toast.success('Book deleted successfully');
        
        // אופציונלי: טעינה מחדש ברקע כדי לוודא סנכרון
        // loadData(); 
      } catch (error) {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
                        ...
                        onClick={async () => {
                          if (window.confirm(`Delete user "${user.name}"?`)) {
                            try {
                              ...
                              setUsers(prev => prev.filter(u => u._id !== user._id)); // עדכון מיידי
      ...
      {/* ✅ תיקון 2: עיצוב הדיאלוג (רקע לבן, צל, גבולות) */}
