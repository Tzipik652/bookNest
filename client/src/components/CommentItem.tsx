...
function CommentItem({ comment, isBookOwner, currentUserId, onDelete, onReaction }: CommentItemProps) {
    ...
    return (
        <Card elevation={0} sx={{ border: '1px solid', borderColor: 'divider', borderRadius: 2, transition: 'all 0.2s', '&:hover': { boxShadow: '0 4px 12px rgba(0,0,0,0.08)' } }}>
            <CardContent sx={{ p: 3 }}>
                <Stack spacing={2}>
                    {/* Header */}
                    ...
                    {/* Text */}
                    <Typography variant="body1" sx={{ pl: 7, lineHeight: 1.7, color: 'text.primary' }}>
                        {comment.text}
                    </Typography>
